...
const SVG_PATH = path.join(FRONTEND_DIR, 'public', 'logo.svg');
const PWA_IMAGES_DIR = path.join(FRONTEND_DIR, 'public', 'images');
...
async function generateIcon(page, svgContent, outputPath, size) {
  ...
  await page.screenshot({ path: outputPath, type: 'png', timeout: 60000 });
...
async function main() {
  ...
  // Launch browser - use system Chrome or specify channel
  ...
  try {
    ...
    const faviconPath = path.join(FRONTEND_DIR, 'public', 'favicon.png');
