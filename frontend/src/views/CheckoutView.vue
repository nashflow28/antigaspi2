    ...
    <div class="border-b border-neutral-200/70 bg-white/80 backdrop-blur dark:border-neutral-800/70 dark:bg-neutral-900/80">
      <div class="container px-4 sm:px-6 lg:px-8 mx-auto px-6 py-10">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p class="inline-flex items-center gap-2 rounded-full bg-primary-100/70 px-4 py-3 text-responsive-sm font-medium text-primary-emphasis dark:bg-primary-500/20 dark:text-primary-100">
              <CreditCard class="h-5 w-5" />
              Finaliser ma réservation
            </p>
            <h1 class="mt-3 text-display-sm font-semibold tracking-tight text-heading dark:text-neutral-50">Paiement sécurisé AntiGaspi</h1>
            <p class="mt-2 max-w-full sm:max-w-2xl text-body dark:text-neutral-300">
              Vérifiez vos informations, choisissez votre mode de paiement et confirmez votre panier surprise en toute sérénité.
            </p>
          </div>
          <div class="flex items-center gap-2 text-responsive-sm text-muted dark:text-placeholder" aria-hidden="true">
            ...
            <span>Résumé</span>
            <span class="text-placeholder dark:text-body">—</span>
    ...
    <main class="container px-4 sm:px-6 lg:px-8 mx-auto grid gap-6 sm:gap-8 px-6 py-8 sm:py-10 lg:py-12 lg:grid-cols-[2fr_1fr]">
      <section class="space-y-6" aria-label="Formulaire de paiement">
        <Card class="bg-white/90 dark:bg-neutral-900/90">
          <template #header>
            <h2 id="checkout-pickup" class="text-responsive-xl font-semibold text-heading dark:text-neutral-50">Informations de retrait</h2>
          </template>
          <div class="grid gap-4 md:grid-cols-2" role="group" aria-labelledby="checkout-pickup">
            <label class="flex flex-col gap-2 text-responsive-sm text-body dark:text-neutral-200">
              Nom complet
              <input
                v-model="contactName"
                type="text"
                class="rounded-2xl border border-neutral-200 bg-white/80 px-4 py-3 text-heading shadow-inner transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-neutral-700 dark:bg-neutral-900/80 dark:text-neutral-50 dark:focus-visible:ring-offset-neutral-950"
        ...
        <Card class="bg-white/90 dark:bg-neutral-900/90">
          <template #header>
            <div class="flex items-center justify-between gap-4">
              <h2 id="checkout-payment" class="text-responsive-xl font-semibold text-heading dark:text-neutral-50">Mode de paiement</h2>
              <span class="text-responsive-sm text-muted dark:text-placeholder">Frais transparents, confirmation immédiate</span>
            </div>
          </template>
          ...
          >
            ...
            >
              <span class="text-responsive-sm font-semibold text-heading dark:text-neutral-50">{{ option.label }}</span>
              <span class="text-responsive-xs text-muted dark:text-neutral-300">{{ option.description }}</span>
            </button>
          </div>

          <div class="mt-4 flex items-center gap-2 rounded-2xl bg-primary-50 px-4 py-3 text-responsive-sm text-primary-emphasis dark:bg-primary-500/15 dark:text-primary-100">
        ...
        <Card class="bg-white/90 dark:bg-neutral-900/90">
          <template #header>
            <h2 id="checkout-terms" class="text-responsive-xl font-semibold text-heading dark:text-neutral-50">Conditions</h2>
          </template>
          <div class="space-y-3 text-responsive-sm text-body dark:text-neutral-300" role="group" aria-labelledby="checkout-terms">
            <label class="flex items-stretch sm:items-start gap-3">
              <input v-model="termsAccepted" type="checkbox" class="mt-1 h-5 w-5 rounded border-neutral-300 text-primary focus-visible:ring-primary-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-neutral-700 dark:bg-neutral-900 dark:text-primary-400 dark:focus-visible:ring-offset-neutral-950">
              <span>J'accepte les conditions d'annulation AntiGaspi et m'engage à récupérer mon panier dans le créneau choisi.</span>
            </label>
            <label class="flex items-stretch sm:items-start gap-3">
              <input v-model="subscribeNotifications" type="checkbox" class="mt-1 h-5 w-5 rounded border-neutral-300 text-primary focus-visible:ring-primary-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-neutral-700 dark:bg-neutral-900 dark:text-primary-400 dark:focus-visible:ring-offset-neutral-950">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
          <Button variant="ghost" class="text-muted dark:text-neutral-300" @click="router.push({ name: 'cart' })">
            Revenir au panier
          </Button>
          <Button
            :loading="processing"
      ...
      <aside class="space-y-6" aria-label="Résumé et assistance">
        <Card class="bg-white/90 dark:bg-neutral-900/90">
          <template #header>
            <h2 class="text-responsive-xl font-semibold text-heading dark:text-neutral-50">Résumé du panier</h2>
          </template>
          <ul class="space-y-4 text-responsive-sm text-body dark:text-neutral-300">
            <li
              v-for="item in items"
              :key="item.id"
            ...
            >
              <div>
                <p class="font-semibold text-heading-secondary dark:text-neutral-100">{{ item.name }}</p>
                <p v-if="item.merchantName" class="text-responsive-xs text-muted dark:text-placeholder">{{ item.merchantName }}</p>
              </div>
              <div class="text-right">
                <p class="font-semibold text-heading-secondary dark:text-neutral-100">{{ formatPrice(item.price * item.quantity) }}</p>
                <p class="text-responsive-xs text-placeholder dark:text-muted">{{ item.quantity }} × {{ formatPrice(item.price) }}</p>
              </div>
            </li>
          </ul>
          <div class="mt-6 space-y-2 border-t border-neutral-200 pt-4 text-responsive-sm dark:border-neutral-800">
            <div class="flex items-center justify-between">
              ...
              <span class="font-semibold text-heading-secondary dark:text-neutral-100">{{ formattedTotal }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span>Frais de service</span>
              <span class="text-placeholder dark:text-muted">Offerts</span>
            </div>
            <div class="flex items-center justify-between text-responsive-base font-semibold text-heading dark:text-neutral-50">
