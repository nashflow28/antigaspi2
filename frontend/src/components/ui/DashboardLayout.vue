<template>
  <div :class="wrapperClasses">
    <Motion
      ...
      tag="div"
      class="fixed inset-0 z-40 bg-neutral-50-dark/45 backdrop-blur-sm lg:hidden sm:block"
      :initial="{ opacity: 0 }"
    ...
    >
      <div class="flex h-full flex-col">
        <div class="flex items-center gap-3 border-b border-neutral-200/70 px-4 py-6 dark:border-neutral-800/80">
          <slot name="sidebar-brand" :brand="sidebar.brand">
            <div
              v-if="sidebar.brand.logo"
              class="flex h-8 w-8 items-center justify-center rounded bg-primary-500/15 text-primary-900 dark:text-primary-200"
            >
              <component :is="sidebar.brand.logo" />
            </div>
            <h1 class="text-lg font-semibold text-primary-800 dark:text-primary-200">{{ sidebar.brand.name }}</h1>
          </slot>
        </div>
          ...
          <template v-for="(item, index) in sidebar.navigation" :key="item.label">
            ...
            >
              <slot name="sidebar-item" :item="item" :index="index">
                ...
                <span
                  v-if="item.badge"
                  class="rounded-full bg-primary-500/20 px-3 py-3 text-xs font-semibold text-primary-900 dark:text-primary-100"
                >
                  {{ item.badge }}
                </span>
              </slot>
      ...
      >
        <div class="px-3 sm:px-4 lg:px-6">
          <div class="flex h-10 items-center justify-start sm:justify-between gap-3">
            <button
              type="button"
              class="rounded p-2 text-primary-900 transition-colors hover:bg-primary-200/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400 lg:hidden sm:block"
            ...
            <div class="flex flex-1 items-center justify-center sm:justify-end gap-3">
              ...
              <slot name="header-user" :user="header.user">
                <div class="flex items-center gap-3 rounded bg-primary-500/10 px-3 py-3 text-left text-primary-800 transition-colors hover:bg-primary-500/15 dark:bg-primary-500/10 dark:text-primary-100">
                  <img
                  ...
                  >
                  <span v-else class="flex h-10 w-9 items-center justify-center rounded-full bg-primary-500 text-white">
                    {{ header.user.name.charAt(0).toUpperCase() }}
                  </span>
                  <div class="hidden sm:block sm:block">
                    <p class="text-sm font-semibold">{{ header.user.name }}</p>
                    <p class="text-xs text-primary-900/80 dark:text-primary-100/70">{{ header.user.email }}</p>
                  </div>
                </div>
              </slot>
            </div>
...
const asideClasses = computed(() =>
  [
    'fixed inset-y-0 left-0 z-50 w-72 transform border-r border-primary-500/10 bg-white/95 shadow-lg dark:border-neutral-800 dark:bg-neutral-900/95',
...
const navigationItemClasses = (item: NavigationEntry) =>
  [
    ...
    item.active
      ? 'bg-primary-500/10 text-primary-900 shadow-inner dark:text-primary-100'
      : 'text-neutral-500 hover:bg-primary-200/20 hover:text-primary-900 dark:text-neutral-500 dark:hover:text-primary-100'
  ]
    .filter(Boolean)
    .join(' ')
