<template>
  <div class="surprise-basket-detail">
    <!-- Header -->
    <div class="bg-white rounded border border-neutral-200 p-6 mt-4">
      <div class="flex items-stretch sm:items-start justify-start sm:justify-between">
        <div class="flex-1">
          <div class="flex items-center space-y-2 sm:space-x-3 mb-4">
            <h1 class="text-xl font-semibold text-neutral-900">{{ basket.name }}</h1>
            <span
          ...
          <p v-if="basket.description" class="text-neutral-700 mt-3">
            {{ basket.description }}
          </p>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            <div class="bg-primary-50 rounded p-3">
              ...
              <p class="text-lg font-semibold text-primary-900">{{ formatCurrency(basket.discounted_price) }}</p>
            </div>
            <div class="bg-green-50 rounded p-3">
              <p class="text-sm text-green-600 font-medium">Économies</p>
              <p class="text-lg font-semibold text-primary-900">{{ formatCurrency(basket.basket_savings || 0) }}</p>
            </div>
            <div class="bg-primary-50 rounded p-3">
            ...
            <div class="bg-orange-50 rounded p-3">
              <p class="text-sm text-primary-600 font-medium">Stock</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-6">
      ...
      <div class="lg:col-span-2">
        ...
        <div class="bg-white rounded border border-neutral-200 p-6">
          <h2 class="text-lg font-semibold text-neutral-900 mt-3">Produits inclus</h2>
          ...
          <div v-if="basket.surprise_basket_items && basket.surprise_basket_items.length > 0" class="space-y-4">
            <div
              ...
              :key="item.id"
              class="flex items-center space-y-4 sm:space-x-4 p-4 bg-neutral-50 rounded"
            >
              <!-- Product Image -->
              <img
              ...
              <div
                v-else
                class="w-12 h-10 bg-neutral-200 rounded flex items-center justify-center flex-shrink-0"
              >
            ...
            <!-- Total Summary -->
            <div class="border-t border-neutral-200 padding-t-lg">
              <div class="bg-green-50 border border-primary-200 rounded p-4">
                <div class="flex justify-start sm:justify-between items-center text-sm mt-2">
                  ...
                  <span class="font-semibold text-primary-900">{{ formatCurrency(basket.total_original_value || 0) }}</span>
                </div>
                <div class="flex justify-start sm:justify-between items-center text-sm mt-2">
                  <span class="text-green-700">Prix du panier:</span>
                  <span class="font-semibold text-primary-900">{{ formatCurrency(basket.discounted_price) }}</span>
                </div>
                <div class="flex justify-start sm:justify-between items-center text-lg border-t border-primary-200 padding-t-sm">
          ...
          <div v-else class="text-left sm:text-center py-8 sm:py-12 lg:py-16 text-neutral-500">
            <Package class="w-12 h-10 mx-auto mt-3 text-neutral-500" />
            <h3 class="text-lg font-medium text-neutral-900 mt-2">Aucun produit</h3>
      ...
      <div class="space-y-6">
        <!-- Basic Information -->
        <div class="bg-white rounded border border-neutral-200 p-6">
          <h3 class="text-lg font-semibold text-neutral-900 mt-3">Informations</h3>

          <div class="space-y-4">
            <!-- Category -->
            <div v-if="basket.category">
              <p class="text-sm font-medium text-neutral-700">Catégorie</p>
              <p class="text-neutral-900">{{ basket.category.name }}</p>
            </div>

            <!-- Mystery Description -->
            <div v-if="basket.surprise_description">
              <p class="text-sm font-medium text-neutral-700">Description mystère</p>
              <p class="text-neutral-900">{{ basket.surprise_description }}</p>
            </div>

            <!-- Item Range -->
            <div v-if="basket.min_items || basket.max_items">
              <p class="text-sm font-medium text-neutral-700">Nombre d'articles</p>
            ...
            <div v-if="basket.expiration_date">
              <p class="text-sm font-medium text-neutral-700">Date d'expiration</p>
              <p class="text-neutral-900">{{ formatDate(basket.expiration_date) }}</p>
            </div>

            <!-- Creation Date -->
            <div>
              <p class="text-sm font-medium text-neutral-700">Créé le</p>
              <p class="text-neutral-900">{{ formatDateTime(basket.created_at) }}</p>
            </div>

            <!-- Last Update -->
            <div v-if="basket.updated_at !== basket.created_at">
              <p class="text-sm font-medium text-neutral-700">Dernière modification</p>
        ...
        <!-- Image Preview -->
        <div v-if="basket.image_url" class="bg-white rounded border border-neutral-200 p-6">
          <h3 class="text-lg font-semibold text-neutral-900 mt-3">Image</h3>
          <img
        ...
        <!-- Quick Actions -->
        <div class="bg-white rounded border border-neutral-200 p-6">
          <h3 class="text-lg font-semibold text-neutral-900 mt-3">Actions</h3>

          <div class="space-y-2">
            <button
              class="w-full px-3 py-3 bg-primary-600 text-white rounded hover:transition-colors flex items-center justify-center"
              @click="$emit('edit', basket)"
            >
            ...
            <button
              :disabled="updating"
              class="w-full px-3 py-3 border border-neutral-300 text-neutral-800 rounded hover:transition-colors flex items-center justify-center disabled:opacity-50"
              @click="toggleStatus"
            >
