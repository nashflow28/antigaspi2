<template>
  <div class="bg-white rounded shadow-lg border border-neutral-100">
      ...
      <div class="flex items-center justify-start sm:justify-between">
          ...
          <div>
            ...
            <div v-if="stats && stats.total_reviews > 0" class="flex items-center space-y-4 sm:space-x-2">
              <div class="flex items-center">
                ...
                <span class="ml-1 text-sm font-medium text-neutral-900">{{ stats.average_rating }}</span>
              </div>
              <span class="text-xs text-neutral-500">sur {{ stats.total_reviews }} avis</span>
            </div>
          </div>
        </div>
        ...
        <div class="relative">
          <select
            ...
            class="text-sm border border-neutral-300 rounded px-3 py-3 focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
            @change="() => fetchReviews()"
          >
    ...
    <!-- Rating Statistics -->
    <div v-if="stats && stats.total_reviews > 0" class="px-4 py-4 border-b border-neutral-200">
      <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
        ...
        >
          <div class="flex items-center space-y-4 sm:space-x-2 text-xs text-neutral-700 w-12">
            <span>{{ rating.rating }}</span>
            <Star class="w-xs h-3 text-yellow-400 fill-yellow-400" />
          </div>
          <div class="flex-1 bg-neutral-200 rounded-full h-4">
            <div
              class="bg-yellow-400 h-4 rounded-full transition-all duration-300"
              :style="{ width: rating.percentage + '%' }"
            />
          </div>
          <span class="text-xs text-neutral-500 w-12">{{ rating.count }}</span>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <div v-if="reportingReviewId === review.id" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[120]">
          <div class="bg-white rounded p-6 max-w-xl w-full mx-lg">
            <h3 class="text-lg font-semibold text-neutral-900 mt-3">Signaler cet avis</h3>
            ...
            <div class="flex justify-center sm:justify-end space-y-2 sm:space-x-3">
              <button
                class="px-3 py-3 text-neutral-700 bg-neutral-100 rounded hover:transition-colors"
        ...
        <div v-if="replyingToReviewId === review.id" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[120]">
          <div class="bg-white rounded p-6 max-w-xl w-full mx-lg">
            <h3 class="text-lg font-semibold text-neutral-900 mt-3">Répondre à cet avis</h3>
            ...
            <textarea
              ...
              class="w-full p-3 border border-neutral-300 rounded focus:ring-2 focus:ring-green-500 focus:border-transparent"
              rows="4"
              maxlength="500"
            />
            ...
            <div class="flex justify-center sm:justify-end space-y-2 sm:space-x-3 mt-4">
              ...
              <button
                ...
                class="px-3 py-3 bg-primary-600 text-white rounded hover:transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
