  ...
  <div class="space-y-4">
    <div class="flex space-y-4 sm:space-x-4">
      ...
      <div class="flex-1 min-w-none">
        ...
        <div v-if="review.merchant_response" class="mt-4 p-3 bg-green-50 border border-primary-200 rounded">
        ...
        <div class="flex items-center space-y-2 sm:space-x-3 mt-4">
          ...
          <button
            class="inline-flex items-center px-3 py-3 text-sm text-neutral-700 border border-neutral-300 rounded hover:transition-colors"
        ...
        <div v-if="showDetails" class="mt-4 p-4 bg-neutral-50 rounded">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div>
              <p class="font-medium text-neutral-900">Informations utilisateur</p>
              <p class="text-neutral-700">Email: {{ review.user.email }}</p>
              <p class="text-neutral-700">ID: {{ review.user.id }}</p>
            </div>
            <div>
              <p class="font-medium text-neutral-900">Informations commerçant</p>
              <p class="text-neutral-700">Propriétaire: {{ review.merchant.owner_name }}</p>
              <p class="text-neutral-700">ID: {{ review.merchant.id }}</p>
            </div>
            <div>
              <p class="font-medium text-neutral-900">Date de création</p>
              <p class="text-neutral-700">{{ formatDate(review.created_at) }}</p>
            </div>
            <div v-if="review.product">
              <p class="font-medium text-neutral-900">Produit</p>
              <p class="text-neutral-700">{{ review.product.name }} (ID: {{ review.product.id }})</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="successMessage" class="p-3 bg-green-50 border border-primary-200 rounded">
      <div class="flex items-center">
